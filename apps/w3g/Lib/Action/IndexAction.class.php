...
class IndexAction extends BaseAction {
	...
	public function weibo_list($uid = 0) {
		...
		foreach ( $this->tVar ['weibolist'] as $key => $value ) { // optimize data
			...
			} else if ($this->tVar ['weibolist'] [$key] ['type'] === 'postfile') {
			...
			} else if ($this->tVar ['weibolist'] [$key] ['type'] === 'weiba_repost') {
	...
	public function retweet() {
		...
		} else if (($detail ['api_source'] ['is_del'] === '1' || $detail ['transpond_data'] ['api_source'] ['is_del'] === '1') && $detail ['type'] === 'repost') {
			$detail ['type'] = 'repost-removed';
		} else if ($detail ['type'] === 'postfile') {
		...
		} else if ($detail ['type'] === 'weiba_repost') {
	...
	public function weibo() {
		...
		if ($userPrivacy === true || $userPrivacy ['space'] == 0) {
			...
			foreach ( $this->tVar ['weibolist'] as $key => $value ) { // optimize data
				...
				} else if (($this->tVar ['weibolist'] [$key] ['api_source'] ['is_del'] === '1' || $this->tVar ['weibolist'] [$key] ['transpond_data'] ['api_source'] ['is_del'] === '1') && $this->tVar ['weibolist'] [$key] ['type'] === 'repost') {
					$this->tVar ['weibolist'] [$key] ['type'] = 'repost-removed';
				} else if ($this->tVar ['weibolist'] [$key] ['type'] === 'postfile') {
					foreach ( $this->tVar ['weibolist'] [$key] ['attach'] as $k => $v ) {
						if ($v ['size'] > 1024 && $v ['size'] < 1024 * 1024) {
